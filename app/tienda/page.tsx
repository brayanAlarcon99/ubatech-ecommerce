    ...
    <>
      {checkingStatus ? (
        ...
        <>
          ...
          <div className="relative flex flex-col lg:flex-row min-h-screen gap-0">
            {/* OVERLAY - Solo en móvil cuando está abierto */}
            {mobileFiltersOpen && (
              <div
                className="sm:hidden fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity"
            ...
            {/* CONTENIDO PRINCIPAL */}
            <main className={`flex-1 px-2 sm:px-4 py-6 sm:py-12 w-full lg:w-auto transition-all duration-300 ${subcategories.length > 0 ? '' : 'lg:max-w-7xl lg:mx-auto'}`}>
              {error && (
                <div className="mb-8 p-3 sm:p-4 bg-red-100 border border-red-400 text-red-700 rounded text-sm sm:text-base">
                  {error}
                </div>
              )}
              ...
              {loading ? (
                    ...
                    <div className="space-y-6 sm:space-y-8">
                      {categorySections.map(([categoryName, categoryProducts], index) => (
                        <div key={categoryName}>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-8">
                ...
                <p className="col-span-full text-center text-gray-500 py-12 text-sm sm:text-base">
